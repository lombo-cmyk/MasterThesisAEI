...
private:
    ...
    void StopMeasuring();
    void SetPointerAndRead(const std::uint16_t ptrAddress, std::uint8_t* data, std::size_t len);
    void SetPointerAndWrite(const std::uint16_t ptrAddress, uint8_t* data, uint8_t len);
