...
uint8_t CalcCrc(std::uint8_t data_0, std::uint8_t data_1) {
...
void ParticlesSensor::PerformReadOut() {
    ...
    for (const auto& d: data){
        std::cout << "some data: " << static_cast<int>(d) << std::endl;
    }
}
...
void ParticlesSensor::SetPointerAndRead(const std::uint16_t ptrAddress, std::uint8_t* data, std::size_t len) {
...
void ParticlesSensor::SetPointerAndWrite(const std::uint16_t ptrAddress,
    ...
    for (std::size_t i = 0; i < len; i += 2) {
        ...
        i2c_master_write_byte(commandHandle, CalcCrc(data[i], data[i+1]), ACK_CHECK);
    }
